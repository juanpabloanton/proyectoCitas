...
class AdminerEditCalendar extends Adminer\Plugin
{
	...
	function editInput($table, $field, $attrs, $value)
	{
		if (preg_match("~date|time~", $field["type"])) {
			...
			return "<input id='fields-" . Adminer\h($field["field"]) . "' value='" . Adminer\h($value) . "'" .
				(is_numeric($field["length"]) && $field["length"] > 0 ? " data-maxlength='" . (int) $field["length"] . "'" : "") .
				"$attrs>" . Adminer\script(
